...
export async function handleRequest(req: Request, adminClient: AdminClient = createAdminClient()): Promise<Response> {
  ...
  const { data: lead, error: leadErr } = await adminClient
    .from("leads")
    .select("id, status, team, created_by, person_responsible_id, reviewer_id, recommending_authority_id, handled_by_dgm_id, assigned_ba_id")
    .eq("id", lead_id)
    .maybeSingle();
  if (leadErr || !lead) return jsonRes(req, 404, { error: "Lead not found." });

  if (!canViewLead(caller, lead)) return jsonRes(req, 403, { error: "You do not have access to this lead." });
